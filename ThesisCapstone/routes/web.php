...
Route::middleware('auth')->group(function () {
    ...
    */
    Route::prefix('user')->group(function () {
        ...
        // Fetch current user info (for modal auto-fill)
        ...
        Route::get('/application-status', [ServiceProviderController::class, 'applicationStatus'])
            ->name('user.application-status');
        Route::post('/apply-service-provider', [ServiceProviderController::class, 'apply'])
            ->name('user.apply-service-provider');
     ...
     // Service Provider Details
        Route::get('/service-provider-details', [ServiceProviderController::class, 'details'])
            ->name('user.service-provider-details');
    ...
    Route::post('/complete-job/{id}', [ServiceProviderController::class, 'completeJob']); // <-- add this
    });
    ...
    Route::middleware(['auth'])->group(function() {
        // Get all requests that are awaiting material
        Route::get('/procurement/requests-awaiting-material', [ProcurementController::class, 'awaitingMaterial']);

        // Mark request as job ready
        Route::post('/procurement/mark-job-ready/{id}', [ProcurementController::class, 'markJobReady']);
    });
    ...
    Route::prefix('hr')->middleware('role:hr')->group(function () {
        ...
        Route::patch('service-requests/{serviceRequest}/status', [HrDashboardController::class, 'updateRequestStatus'])
            ->name('hr.service-requests.status');
        Route::get('service-requests/accepted', [HrRequestStatusController::class, 'accepted'])
            ->name('hr.service-requests.accepted');
        Route::get('service-requests/rejected', [HrRequestStatusController::class, 'rejected'])
            ->name('hr.service-requests.rejected');
...
// routes/web.php
Route::middleware(['auth', 'dashboard.redirect'])->group(function () {
    Route::get('/user/dashboard', fn() => Inertia::render('UserDashboard'))
        ->name('user.dashboard');

    Route::get('/service-provider/dashboard', fn() => Inertia::render('ServiceProviderDashboard'))
        ->name('service-provider.dashboard');
});

Route::middleware('auth')->get( '/service-provider/dashboard-data', [ServiceProviderController::class, 'dashboard'] );

Route::post('/business/service-providers/{provider}/review', [ServiceProviderController::class, 'review'])->name('sp.review');
    ...
    */
    ...
    Route::post('/service-provider/update-request/{id}', [ServiceProviderController::class, 'updateRequest'])
    ->middleware('auth'); // 
    /*
    |--------------------------------------------------------------------------
    ...
    |--------------------------------------------------------------------------
    */
    Route::get('/hr', fn () => Inertia::render('HrDashboard'))->middleware('role:hr')->name('hr.dashboard');
    ...
    Route::get('/employee/dashboard', fn () => Inertia::render('EmployeeDashboard'))->middleware('role:employee')->name('employee.dashboard');
    Route::get('/employee/profile', fn () => Inertia::render('EmployeeProfile'))->middleware('role:employee')->name('employee.profile');
    Route::get('/employee/notifications', fn () => Inertia::render('EmployeeNotifications'))->middleware('role:employee')->name('employee.notifications');
    Route::get('/employee/payslips', fn () => Inertia::render('EmployeePayslips'))->middleware('role:employee')->name('employee.payslips');
    Route::get('/employee/assigned-requests', fn () => Inertia::render('EmployeeAssignedRequests'))->middleware('role:employee')->name('employee.assigned-requests');
    Route::prefix('employee')->middleware('role:employee')->group(function () {
        ...
        Route::patch('/assigned-requests/{serviceRequest}', [EmployeeDashboardController::class, 'updateAssignedRequest'])
            ->name('employee.assigned-requests.update');
    });
